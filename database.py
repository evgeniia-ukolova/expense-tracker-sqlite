...
def create_table() -> None:         # создание таблицы
    connection = get_connection()         # подключаемся к базе. Если файла нет, SQLite создаст его.
...
def get_connection() -> sqlite3.Connection:             # функция подключения к базе данных.
    return sqlite3.connect(DATABASE_PATH)               # Создаёт подключение к нашей SQLite-базе и возвращает его той функции
