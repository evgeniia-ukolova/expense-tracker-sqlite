...
def restore_database_backup() -> None:              # восстановление базы данных из резервной копии
    database_path = Path("expenses.db")
    backup_path = Path("backups") / "expenses_backup.db"        # Указывает путь к резервной копии

    if not backup_path.exists():
        print("Резервная копия не найдена")
        return

    confirmation = input(                   # Просит подтверждение, потому что текущая база будет перезаписана
    ...
    shutil.copy2(backup_path, database_path)        # Копирует резервную базу поверх текущей expenses.db.
